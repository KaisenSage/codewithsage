...
export default function ContactSection() {
  ...
  const BOOKING_URL = process.env.NEXT_PUBLIC_BOOKING_URL || "https://calendly.com/kvngbabstar/30min";
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center bg-gradient-to-br from-white to-purple-50">
      <div className="w-full max-w-6xl mx-auto px-4 pt-8">
        {/* Modern Back to Home Button */}
        <Link
          href="/"
          className="absolute left-4 top-6 md:left-8 md:top-8 z-50 inline-flex items-center gap-2 px-4 py-2 rounded-lg font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 hover:text-blue-900 shadow-sm transition"
        >
          ...
          <div className="md:w-1/3 bg-gradient-to-br from-blue-50 to-purple-100 p-8 flex flex-col gap-6">
            ...
            <div className="mt-4 p-4 bg-white rounded-lg border shadow-sm">
              ...
              <p className="text-sm text-gray-600 mt-2">
                Book a free 15-minute consultation to discuss your project, timeline, and budget. Quick, no-pressure chat.
              </p>
              ...
              <div className="mt-3 text-xs text-gray-500">
                Prefer email? Reply to contact.codewithsage@gmail.com with your availability.
              </div>
            </div>
          </div>
          ...
          <div className="md:w-2/3 p-8">
            ...
            {state.succeeded && (
              <div className="mb-4 flex flex-col items-center justify-center">
                <div className="flex items-center gap-2 bg-gradient-to-r from-blue-400 to-blue-600 text-white px-4 py-3 rounded-lg shadow-lg">
                  <FaCheckCircle className="text-2xl" />
                  <span className="font-semibold text-lg">Your message has been sent!</span>
                </div>
                <div className="mt-2 text-blue-700 text-base text-center">Thank you for contacting us. We’ll get back to you as soon as possible.</div>
              </div>
            )}

            {Array.isArray(state.errors) && state.errors.length > 0 && (
              <div className="mb-4 text-red-600 font-semibold">{state.errors.map((err, i) => <div key={i}>{err.message}</div>)}</div>
            )}
            ...
            {!state.succeeded && (
              <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col mb-2">
                  ...
                  <input type="text" name="name" id="name" required placeholder="John Doe" className="border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700" />
                  <ValidationError prefix="Name" field="name" errors={state.errors} />
                </div>

                <div className="flex flex-col mb-2">
                  <label htmlFor="email" className="font-medium mb-1 text-gray-700">Email Address *</label>
                  <input type="email" name="email" id="email" required placeholder="john@example.com" className="border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700" />
                  <ValidationError prefix="Email" field="email" errors={state.errors} />
                </div>

                <div className="flex flex-col mb-2">
                  <label htmlFor="phone" className="font-medium mb-1 text-gray-700">Phone Number</label>
                  <input type="text" name="phone" id="phone" placeholder="+234 123 456 7890" className="border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700" />
                  <ValidationError prefix="Phone" field="phone" errors={state.errors} />
                </div>

                <div className="flex flex-col mb-2">
                  <label htmlFor="subject" className="font-medium mb-1 text-gray-700">Subject *</label>
                  <input type="text" name="subject" id="subject" required placeholder="Project Inquiry" className="border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700" />
                ...
                <div className="flex flex-col md:col-span-2 mb-4">
                  ...
                  <textarea name="message" id="message" required rows={4} placeholder="Tell us about your project..." className="border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700"></textarea>
                  <ValidationError prefix="Message" field="message" errors={state.errors} />
                </div>

                <div className="md:col-span-2">
                  <button type="submit" className={`flex items-center justify-center gap-2 w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold py-3 rounded-lg shadow hover:from-blue-700 hover:to-purple-700 transition ${loading || state.submitting ? "opacity-60 cursor-not-allowed" : ""}`} disabled={loading || state.submitting}>
